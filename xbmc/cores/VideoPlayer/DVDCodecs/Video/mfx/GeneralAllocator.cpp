...
CGeneralAllocator::CGeneralAllocator() = default;

CGeneralAllocator::~CGeneralAllocator() = default;

mfxStatus CGeneralAllocator::Init(mfxAllocatorParams *pParams)
{
  mfxStatus sts;
...
mfxStatus CGeneralAllocator::Close()
{
  mfxStatus sts;
  if (m_HWAllocator.get())
  {
...
mfxStatus CGeneralAllocator::LockFrame(mfxMemId mid, mfxFrameData *ptr)
{
...
mfxStatus CGeneralAllocator::UnlockFrame(mfxMemId mid, mfxFrameData *ptr)
{
  if (isHWMid(mid) && m_HWAllocator.get())
...
mfxStatus CGeneralAllocator::GetFrameHDL(mfxMemId mid, mfxHDL *handle)
{
  if (isHWMid(mid) && m_HWAllocator.get())
...
mfxStatus CGeneralAllocator::ReleaseResponse(mfxFrameAllocResponse *response)
{
...
mfxStatus CGeneralAllocator::AllocImpl(mfxFrameAllocRequest *request, mfxFrameAllocResponse *response)
{
  mfxStatus sts;
...
void CGeneralAllocator::StoreFrameMids(bool isD3DFrames, mfxFrameAllocResponse *response)
{
  for (mfxU32 i = 0; i < response->NumFrameActual; i++)
    m_Mids.insert(std::pair<mfxHDL, bool>(response->mids[i], isD3DFrames));
}

bool CGeneralAllocator::isHWMid(mfxHDL mid)
{
