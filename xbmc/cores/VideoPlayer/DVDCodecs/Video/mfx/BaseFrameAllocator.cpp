...
mfxStatus CBaseFrameAllocator::FreeFrames(mfxFrameAllocResponse *response)
{
  ...
  const auto i = std::find_if(m_ExtResponses.begin(), m_ExtResponses.end(), [&response](auto& a) { return IsSame()(a, *response); });
  ...
  const auto i2 = std::find_if(m_responses.begin(), m_responses.end(), [&response](auto& a) { return IsSame()(a, *response); });
