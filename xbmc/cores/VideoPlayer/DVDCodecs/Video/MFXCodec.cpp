...
#define MSDK_CHECK_RESULT(p, x)                                                                    \
  {                                                                                                \
    if ((x) > (p))                                                                                 \
    {                                                                                              \
      CLog::Log(LOGERROR, "{}: error code {} ({})", __FUNCTION__, p, __LINE__);                    \
      return false;                                                                                \
    }                                                                                              \
  }
...
{
  std::unique_lock lock(m_BufferCritSec);
  ...
  if (!pBuffer)
    CLog::Log(LOGERROR, "no more free buffers available ({} total)", m_BufferQueue.size());
...
{
  std::unique_lock lock_ctx(m_section);
  std::unique_lock lock_buff(m_BufferCritSec);
...
{
  m_frameAllocator = std::make_unique<MFX::CGeneralAllocator>();
...
// MVC Decoder
//-----------------------------------------------------------------------------
std::unique_ptr<CDVDVideoCodec> CMFXCodec::Create(CProcessInfo& processInfo)
{
  return std::unique_ptr<CDVDVideoCodec>(new CMFXCodec(processInfo));
}
...
bool CMFXCodec::Init()
{
  ...
  if (sts != MFX_ERR_NONE)
  {
    ...
    if (sts != MFX_ERR_NONE)
    {
      CLog::Log(LOGERROR, "MSDK not available");
      return false;
    }
  }
  ...
  CLog::Log(LOGINFO, "MSDK Initialized, version {}.{}", m_mfxVersion.Major, m_mfxVersion.Minor);
  if ((m_impl & 0xF00) == MFX_IMPL_VIA_D3D11)
    CLog::Log(LOGDEBUG, "MSDK uses D3D11 API.");
  if ((m_impl & 0xF00) == MFX_IMPL_VIA_D3D9)
    CLog::Log(LOGDEBUG, "MSDK uses D3D9 API.");
  if ((m_impl & 0xF) == MFX_IMPL_SOFTWARE)
    CLog::Log(LOGDEBUG, "MSDK uses Pure Software Implementation.");
  if ((m_impl & 0xF) >= MFX_IMPL_HARDWARE)
    CLog::Log(LOGDEBUG, "MSDK uses Hardware Accelerated Implementation (default device).");
...
bool CMFXCodec::Open(CDVDStreamInfo& hints, CDVDCodecOptions& options)
{
  if (!CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(
          CSettings::SETTING_VIDEOPLAYER_SUPPORTMVC) || hints.stills)
    return false;
...
bool CMFXCodec::AllocateFrames()
{
  ...
  {
    CLog::Log(LOGWARNING, "SW implementation will be used instead of the HW implementation ({}).",
               sts);
  ...
  CLog::Log(LOGINFO, "using {} {} decoder.", GetName(), bDecOutSysmem ? "SW" : "HW");
  ...
  CLog::Log(LOGDEBUG, "decoder suggested ({}) frames to use. creating ({}) buffers.",
...
bool CMFXCodec::AddData(const DemuxPacket& packet)
{
  ...
  if (!m_bDecodeReady)
  {
    ...
    {
      CLog::Log(LOGDEBUG, "no enought data to init decoder ({})", sts);
      m_buffSize = 0;
      return true;
    }
    if (sts == MFX_ERR_NONE)
    {
      ...
      if (sts < 0)
      {
        CLog::Log(LOGERROR, "error initializing the MSDK decoder ({})", sts);
        return false;
      }
      if (sts == MFX_WRN_PARTIAL_ACCELERATION)
        CLog::Log(LOGWARNING,
                   "SW implementation will be used instead of the HW implementation ({}).", sts);

      if (m_mfxExtMVCSeq.NumView != 2)
      {
        CLog::Log(LOGERROR, "only MVC with two views is supported");
        return false;
      }

      CLog::Log(LOGDEBUG, "initialized MVC with view ids {}, {}", m_mfxExtMVCSeq.View[0].ViewId,
  ...
  XbmcThreads::EndTime<> timeout(25ms); // timeout for DEVICE_BUSY state.
  while (true)
  {
    ...
    if (sts == MFX_WRN_DEVICE_BUSY)
    {
      if (timeout.IsTimePast())
      {
        if (resetCount >= 1)
        {
          CLog::Log(LOGERROR, "decoder did not respond after reset, flushing decoder.");
          Flush();
          return false;
        }
        CLog::Log(LOGWARNING, "decoder did not respond within possible time, resetting decoder.");
  ...
  {
    CLog::Log(LOGERROR, "decoder did not consume any data, discarding");
  ...
  {
    CLog::Log(LOGERROR, "error from mfx call ({})", sts);
